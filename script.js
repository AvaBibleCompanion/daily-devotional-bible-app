document.addEventListener('DOMContentLoaded', () => {
    ...
    const bibleControlElements = [
        ...
        questionInput,
        searchButton,
        // Assuming chapter-navigation divs contain the next/prev chapter buttons
        // We will manage prevChapterTop/nextChapterTop directly in the listener.
        // If there are other elements in the general 'chapter-navigation' div you want to hide, add its ID/class here.
    ...
    const devotionalControlElements = [
        ...
        // prevChapterTop, nextChapterTop (these will be re-wired to nextDay/prevDay)
        // copySelectedTop, shareSelectedTop (these remain visible, but their shareOptionsTop can be hidden initially)
    ].filter(Boolean);
    ...
    // --- NEW: Function to toggle visibility of control elements ---
    function toggleAppMode(isBibleMode) {
        ...
        // Specific handling for chapter navigation buttons (as they change behavior)
        if (prevChapterTop) prevChapterTop.style.display = 'block'; // Always visible, but their click logic changes
        if (nextChapterTop) nextChapterTop.style.display = 'block';
    ...
    function resetToBibleAppView() {
        ...
        toggleAppMode(true); // Set to Bible mode
    }
    ...
    dailyDevotionalButton.addEventListener('click', async () => {
        if (dailyDevotionalButton.textContent === 'Daily Devotional') {
            ...
            toggleAppMode(false); // Set to Devotional mode
        } else {
            resetToBibleAppView();
        }
    });
